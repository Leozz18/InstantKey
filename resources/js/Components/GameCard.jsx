...
export default function GameCard({ product }) {
    ...
    return (
        <Link href={route('products.show', product.slug)} className="game-card group flex flex-col">
            <div className="relative aspect-[4/5] overflow-hidden bg-slate-800">
                ...
                {product.platform && (
                    <div
                        className="absolute top-2 right-2 badge-platform"
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-slate-950 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition" />

                <button
                    onClick={addToCart}
                    className="absolute bottom-3 right-3 flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-r from-brand-500 to-accent-500 text-white shadow-lg shadow-brand-500/40 opacity-0 group-hover:opacity-100 transition hover:scale-110"
            ...
            <div className="p-4 flex-1 flex flex-col">
                <h3 className="font-semibold text-white text-sm line-clamp-2 group-hover:text-brand-400 transition">
                    {product.title}
                </h3>
                {product.genre && (
