...
export default function ProductsEdit({ product, platforms, genres }) {
    ...
    const { data: keyData, setData: setKeyData, post: postKeys, processing: processingKeys, reset: resetKeys, errors: keyErrors } = useForm({
        keys: '',
    });
    ...
    const submitKeys = (e) => {
        e.preventDefault();
        postKeys(route('admin.products.keys.add', product.id), {
            onSuccess: () => resetKeys('keys'),
        });
    };
        ...
        <SiteLayout>
            ...
            <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-10">
                <div className="flex items-center justify-between gap-3 mb-8">
                    <div>
                        <h1 className="text-4xl font-extrabold">Modifica Prodotto</h1>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div className="lg:col-span-2">
                        <div className="card">
                            <h2 className="text-xl font-bold mb-6">Dettagli Prodotto</h2>
                            <form onSubmit={submit} className="space-y-6">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium text-slate-300 mb-1">Prezzo Originale (€)</label>
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium text-slate-300 mb-1">URL Immagine</label>
                                        <input type="url" className="input-field" value={data.image_url} onChange={e => setData('image_url', e.target.value)} />
                                    </div>

                                    <div>
                                        <label className="block text-sm font-medium text-slate-300 mb-1">URL Cover</label>
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium text-slate-300 mb-1">ID YouTube</label>
                                        <input type="text" className="input-field" value={data.youtube_id} onChange={e => setData('youtube_id', e.target.value)} />
                                    </div>
                                </div>
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-slate-300 mb-1">Requisiti di Sistema</label>
                    ...
                    <div>
                        <div className="card mb-6 bg-slate-900/50 border-slate-800">
                            <h2 className="text-xl font-bold mb-2">Gestione Stock</h2>
                            <p className="text-slate-400 text-sm mb-6">Aggiungi nuove chiavi a questo prodotto.</p>

                            <div className="flex items-center justify-between p-4 bg-slate-950 rounded-lg border border-slate-800 mb-6">
                                <span className="text-sm text-slate-400">Chiavi Disponibili:</span>
                                <span className={`text-xl font-bold ${product.available_keys === 0 ? 'text-rose-500' : 'text-emerald-500'}`}>
                            ...
                            <form onSubmit={submitKeys} className="space-y-4">
                                <div>
                                    <label className="block text-sm font-medium text-slate-300 mb-1">Nuove Chiavi (una per riga)</label>
                                    <textarea 
                                        className="input-field min-h-[150px] font-mono text-xs" 
                                        placeholder="XXXXX-XXXXX-XXXXX&#10;YYYYY-YYYYY-YYYYY"
                                        value={keyData.keys}
                                        onChange={e => setKeyData('keys', e.target.value)}
                                        required
                                    ></textarea>
                                    {keyErrors.keys && <p className="text-rose-500 text-xs mt-1">{keyErrors.keys}</p>}
                                </div>
                                <button type="submit" disabled={processingKeys || !keyData.keys.trim()} className="btn-primary w-full justify-center">
                                    Aggiungi Chiavi
                                </button>
                            </form>
                        </div>
