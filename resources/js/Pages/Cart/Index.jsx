            ...
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
                ...
                {cart.items.length === 0 ? (
                    <div className="card text-center py-16">
                        ...
                        <Magnet wrapperClassName="mt-6 inline-block" innerClassName="inline-block">
                            <Link href={route('catalog.index')} className="btn-primary inline-flex">
                                Esplora il catalogo
                            </Link>
                        </Magnet>
                    </div>
                    ...
                    <Stepper
                        nextButtonText="Vai al riepilogo"
                                ...
                                {cart.items.map((item) => (
                                    <div key={item.id} className="card flex items-center gap-4 !p-4">
                                        ...
                                        <div className="flex items-center rounded-lg border border-slate-700 bg-slate-900">
                                            <button type="button" onClick={() => updateQty(item.id, item.quantity - 1)} className="px-3 py-1.5 text-slate-400 hover:text-white">−</button>
                                        ...
                                        <div className="text-right shrink-0">
                                            ...
                                            <button type="button" onClick={() => removeItem(item.id)} className="text-xs text-rose-400 hover:text-rose-300 mt-1">Rimuovi</button>
                                        </div>
                                    </div>
                                ))}
                            </div>
                        ...
                        <Step>
                            <div className="space-y-4">
                                <div className="card">
                                    ...
                                    {auth?.user ? (
                                        <Magnet wrapperClassName="mt-6 block w-full" innerClassName="block w-full">
                                            <Link href={route('checkout.index')} className="btn-primary flex w-full items-center justify-center gap-2">
                                        ...
                                        <Magnet wrapperClassName="mt-6 block w-full" innerClassName="block w-full">
                                            <Link href={route('login')} className="btn-primary flex w-full items-center justify-center">
                                ...
                                <div className="card">
                                    ...
                                    {cart.discount ? (
                                        <div className="flex items-center justify-between bg-emerald-500/10 border border-emerald-500/40 rounded-lg p-3">
                                            ...
                                            <button type="button" onClick={removeDiscount} className="text-xs text-emerald-400 hover:text-emerald-200">Rimuovi</button>
                                        </div>
                                    ) : (
